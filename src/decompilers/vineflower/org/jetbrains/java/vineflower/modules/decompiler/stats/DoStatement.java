...
package org.jetbrains.java.vineflower.modules.decompiler.stats;
...
import org.jetbrains.java.vineflower.modules.decompiler.exps.Pattern;
  ...
  public TextBuffer toJava(int indent) {
    ...
    switch (looptype) {
      ...
      case FOR:
        ...
        break;
      case FOR_EACH:
		TextBuffer initStr = initExprent.get(0).toJava(indent);
        buf.appendIndent(indent).append("for (").append(initStr);
        VarType t = incExprent.get(0).getInferredExprType(null); //TODO: Find a better then null? For now just calls it to clear casts if needed
        buf.append(" : ");
		if(t.arrayDim == 0) {
			String s = initStr.toString().split(" ")[0];
			buf.append("(Iterable<").append(s).append(">) ");
		}
		buf.append(incExprent.get(0).toJava(indent)).append(") {").appendLineSeparator();
